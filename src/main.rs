...
fn main() -> Result<()> {
    ...
    let stop: DateTime<Utc> = start+Duration::hours(1);

    let hdffile = File::create(&start.format("%Y-%m-%d %H:%M:%S.h5").to_string())?;
    ...
    let wave_group = hdffile.create_group(&utc.format("%Y-%m-%d %H:%M:%S").to_string())?;
