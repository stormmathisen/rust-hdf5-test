...
fn main() -> std::io::Result<()> {
    ...
    let _handler_status = handler.join().unwrap();
    Ok(())
}
...
fn write_hdf5_thread(controlreceiver: Receiver<bool>, datareceiver: Receiver<DataContainer>) {
    ...
    let mut next_day = start.date().succ().and_hms(0,0,0);
    let mut hdffile = HFile::create(&start.format("/home/storm/Desktop/hdf5rustlocal/%Y-%m-%d %H:%M:%S.h5").to_string()).unwrap();
    ...
    loop {
        ...
        if now > next_day {
            next_day = now.date().succ().and_hms(0,0,0);
            hdffile.close();
            hdffile = HFile::create(&now.format("/home/storm/Desktop/hdf5rustlocal/%Y-%m-%d %H:%M:%S.h5").to_string()).unwrap();
        }
        thread_counter += 1;
